export const values = [
  {
    title: "We deliver value",
    body: "We go above and beyond the checklist to provide meaningful insight, practical guidance, and a better overall experience.",
  },
  {
    title: "We embrace change",
    body: "We continually learn, adopt new technologies, and improve our process to serve clients and community better.",
  },
  {
    title: "We do the right thing",
    body: "We lead with honesty, objectivity, and respect — always putting our clients’ best interests first.",
  },
];

export const services = [
  {
    slug: "buyer-home-inspection",
    title: "Buyer Home Inspection",
    image: "/images/hires/buyer-home-inspection.jpg",
    body: "A detailed, comprehensive understanding of the home’s condition. We inspect major systems and components and deliver a digital report within 24 hours.",
  },
  {
    slug: "new-construction",
    title: "New Construction Phase Inspections",
    image: "/images/hires/new-construction.jpg",
    body: "Independent inspections at pre-pour, rough-in, and final stages help you itemize repairs your builder should address before closing.",
  },
  {
    slug: "11th-month-warranty",
    title: "11th-Month Warranty Inspection",
    image: "/images/hires/11th-month-warranty.jpg",
    body: "Identify issues before your builder’s warranty expires. We document concerns so you can request repairs while coverage is still in place.",
  },
  {
    slug: "foundation",
    title: "Foundation Elevation Readings",
    image: "/images/hires/foundation.jpg",
    body: "Accurate elevation readings help identify foundation movement and provide valuable insight for long-term structural health.",
  },
  {
    slug: "mold",
    title: "Mold Inspection",
    image: "/images/hires/mold.jpg",
    body: "We identify visible signs of moisture and conditions that contribute to mold growth, helping you maintain a healthier home.",
  },
  {
    slug: "pre-listing",
    title: "Pre-Listing Inspection",
    image: "/images/hires/pre-listing.jpg",
    body: "A pre-listing inspection helps you find issues before buyers do — so you can list with confidence and avoid surprises during negotiations.",
  },
  {
    slug: "remodeling",
    title: "Remodeling Consultation",
    image: "/images/hires/remodeling.jpg",
    body: "Get expert insight before you remodel. We evaluate existing conditions, help you plan with confidence, and spot issues early.",
  },
  {
    slug: "sprinkler",
    title: "Sprinkler System Inspection",
    image: "/images/hires/sprinkler.jpg",
    body: "We inspect your irrigation system for proper function, coverage, and leaks so you can protect landscaping and avoid water waste.",
  },
];

export const inspectHighlights = [
  "Exterior",
  "Interior",
  "Plumbing",
  "Electrical",
  "HVAC",
  "Structure",
];

export const inspectSystems = [
  {
    title: "Foundation",
    body: "The integrity of your home’s foundation is critical. We use a high-precision altimeter to check for elevation anomalies around the slab, together with a thorough visual inspection.",
  },
  {
    title: "Roof",
    body: "We walk most roofs and put hands on your investment. We inspect for water damage, improper installation, common deficiencies, and defects that can void a warranty.",
  },
  {
    title: "Electrical",
    body: "We inspect switches, receptacles, electrical panels, light fixtures, and the grounding system so you know what is hiding behind the panel.",
  },
  {
    title: "Mechanical",
    body: "A working AC is necessary in Houston. We inspect ductwork, plenums, unit performance, the condensing unit, evaporator, and the heat exchanger on your furnace.",
  },
  {
    title: "Plumbing",
    body: "We test fixtures and drains in every plumbing room to detect leaks and document deficiencies across traps, vents, pipes, valves, and fixtures.",
  },
  {
    title: "Phase inspections",
    body: "We inspect new construction in three phases: pre-pour foundation, rough-in, and final — so repairs are itemized before closing or during the 11th-month warranty.",
  },
];

export const processSteps = [
  {
    number: "01",
    title: "You schedule",
    body: "Book online, over the phone, or through your real estate agent. We collect a few details, send an easy-to-sign agreement, and we are good to go.",
    image: "/images/hires/you-schedule.jpg",
  },
  {
    number: "02",
    title: "We inspect",
    body: "We arrive early and begin a detailed visual inspection that typically takes two and a half to three hours, covering structural, mechanical, plumbing, and electrical systems.",
    image: "/images/hires/we-inspect.jpg",
  },
  {
    number: "03",
    title: "We report",
    body: "You receive a clear digital report within 24 hours — photos, comments on deficiencies, recommendations, and a summary of repair items to address.",
    image: "/images/hires/we-report.jpg",
  },
  {
    number: "04",
    title: "You decide",
    body: "The decision to purchase is yours. We provide accurate, relevant information about the property and its defects so you can plan for repair costs with confidence.",
    image: "/images/hires/you-decide.jpg",
  },
];

export const included = [
  "Detailed digital report within 24 hours",
  "Photos of every captioned finding",
  "Unbiased, independent assessment",
  "Time to review findings with your inspector",
];

export const faqs = [
  {
    question: "How long does an inspection take?",
    answer:
      "Most inspections take about two and a half to three hours on site, depending on the size and condition of the home.",
  },
  {
    question: "When will I receive my report?",
    answer:
      "Our inspectors deliver a digital report within 24 hours of the inspection, including photos, comments, and a repair summary.",
  },
  {
    question: "Do I need to be present?",
    answer:
      "You are welcome to attend and ask questions, but it is not required. We will still walk you through findings and remain available after the report is delivered.",
  },
  {
    question: "What do you inspect?",
    answer:
      "We examine the major systems of the home: structural, mechanical, plumbing, electrical, roofing, interiors, and visible site conditions. Foundation elevation readings are included.",
  },
  {
    question: "Do you offer additional services?",
    answer:
      "Yes. In addition to buyer inspections we offer phase inspections for new construction, 11th-month warranty inspections, mold inspections, pre-listing inspections, remodeling consultations, and sprinkler system inspections.",
  },
  {
    question: "What areas do you serve?",
    answer:
      "We serve the entire Greater Houston area and beyond — including Katy, The Woodlands, Sugar Land, Pearland, Baytown, Cypress, Spring, and surrounding communities.",
  },
  {
    question: "What languages do you speak?",
    answer:
      "We speak Vietnamese, Cantonese, and Spanish in addition to English — so more families can get clear communication and a better experience.",
  },
];

export const contactFaqs = [
  {
    question: "How soon will I get my report?",
    answer: "Reports are delivered within 24 hours of the inspection.",
    icon: "report",
  },
  {
    question: "Do I need to be present for the inspection?",
    answer: "You’re welcome to attend, but it is not required.",
    icon: "home",
  },
  {
    question: "What areas do you serve?",
    answer:
      "We serve the greater Houston area, including Katy, The Woodlands, Sugar Land, Pearland, Baytown, and surrounding communities.",
    icon: "location",
  },
  {
    question: "What languages do you speak?",
    answer: "We speak Vietnamese, Cantonese, and Spanish in addition to English.",
    icon: "mail",
  },
];
